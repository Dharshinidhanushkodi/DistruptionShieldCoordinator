    ...
    async with AsyncSessionLocal() as session:
        ...
        for task in tasks:
            ...
            if task.start_time < now:
                print(f"Skipping past task: {task.title} ({task.start_time})")
                continue
            ...
            # User snippet hardcodes 1 hour duration
            ...
            # ✅ history log (Mapping History -> DisruptionLog)
            log = DisruptionLog(
                title=task.title,
                old_start=old_start,
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("dashboard_server:app", host="0.0.0.0", port=8001, reload=True)